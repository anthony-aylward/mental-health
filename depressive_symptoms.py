...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description='plot depressive symptoms'
    )
    parser.add_argument(
        'file',
...
def main():
    ...
    life_satisfaction = pd.DataFrame(
        { 
            ...
            'Response': ('Severe', 'Mild to moderate', 'None') * 2,
            'Percent': (12, 24, 64, 12, 18, 70)
        }
    )
    sns.set_color_codes('muted')
    ax = sns.barplot(
        x='Division', y='Percent', data=life_satisfaction, hue='Response',
        palette=(sns.color_palette()[i] for i in (3, 1, 9))
    )
    ax.set_title('Depressive Symptoms')
